...
def test_simple_embed_contains_weekly_goal_and_all_time_high():
    ...
    assert embed["footer"]["text"] == "Vast Revenue Monitor v1.1.3"
    ...
    ath = next(field for field in embed["fields"] if field["name"] == "ATH")
    assert [line.split(":", 1)[0] for line in ath["value"].splitlines()] == [
