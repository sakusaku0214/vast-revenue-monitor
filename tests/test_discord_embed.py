...
def test_japanese_embed_uses_natural_labels_and_clamps_remaining():
    notifier = DiscordNotifier(
        "https://example.invalid", 1, weekly_goal_usd=100, language="ja"
    )
    snapshot = RevenueSnapshot(
        datetime(2026, 7, 25, tzinfo=timezone.utc), 5, 20, 120, 200
    )
    changes = {period.value: Change(0, 0) for period in Period}
    goal = GoalStatus(20, 100, 20, 10, 10, 200, True, snapshot.timestamp, snapshot.timestamp)
    embed = notifier._embed(snapshot, 150, changes, {}, goal, ReportStatus.NORMAL)
    rendered = "\n".join(field["name"] + "\n" + field["value"] for field in embed["fields"])

    assert embed["title"] == "💰 VAST.AI 毎時収益レポート"
    assert "直近区間" in rendered
    assert "目標まで残り: $0.00" in rendered
