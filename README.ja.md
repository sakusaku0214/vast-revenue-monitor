...
```text
09:00:00 以上 ～ 翌日09:00:00 未満
```

収益差分は `(前回の正常取得時刻, 今回の正常取得時刻]` の区間を表し、その**区間の開始時刻**を含む日次期間へ全額を計上します。このため、09:00ちょうどに終了する区間は昨日に属し、09:00時点の本日は **$0.00** から始まります。本日に最初に加算されるのは、開始時刻が09:00以降の区間です。取得が遅れて09:00をまたいだ場合も、根拠のない按分や推計はせず、1つの区間を分割しません。
