...
## Reconfigure weekly goal, language, and detailed reports without reinstalling
...
created before `detailed_report` existed default to Disabled until changed. The
service is restarted only when a setting actually changes.
...
The tool shows current values, accepts Enter to preserve any setting, reviews the proposal, and writes atomically only after confirmation. It preserves credentials, unknown settings, and every state/history file, then restarts `vast-balance.service` only when a setting changes. Older installations without `detailed_report` default to Disabled until changed. Decimal positive goals are accepted; zero, negative, malformed, NaN, and infinite values are rejected. Choose `1`/`en` or `2`/`ja` for language; detailed reports accept `1`/`false`/`no`/`n`/`off` or `2`/`true`/`yes`/`y`/`on`.
