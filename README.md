...
It also asks for the Weekly Goal and notification language. Choose `1` for English
or `2` for Japanese; pressing Enter selects English. Non-interactive installs may
set `NOTIFICATION_LANGUAGE=en` or `NOTIFICATION_LANGUAGE=ja`.
...
- `language`: Discord notification language, exactly `en` or `ja`; missing or
  unsupported legacy values safely fall back to English.
...
positive changes as observed revenue. Daily and monthly totals are aggregated by
their calendar boundaries, while Hourly uses only the newest successful increment
and Weekly displays the current balance.
...
Hourly is the positive balance increment from the latest successful observation
only. It is not normalized or extrapolated to 60 minutes, so manual and delayed
runs report their actual latest interval. Daily assigns a positive delta to the business day that
contains the later observation. If one increase spans 09:00 JST, the API cannot
reveal when it occurred, so the entire delta is assigned to the new day.
...
[Y/n]` choice. `Y` completely removes all generated files. `N` first creates a
private backup containing `config.json` and `state/`; only after that succeeds does
it perform the same complete removal. No orphaned service UID/GID files remain.
...
The supported installed command is `sudo /opt/vast-revenue-monitor/reconfigure.sh`.
It preserves all unrelated settings and secrets, lets you change the Weekly Goal
and notification language, atomically replaces the configuration, and restarts the
service. Press Enter to retain either current value.
...
> **Sensitive data:** Backup archives contain `config.json`, including the Vast.ai
> API key and Discord webhook. Store archives securely with restricted access, do
> not commit them to Git, and do not upload them to public file-sharing services.
...
sudo bash install.sh --reconfigure
sudo bash install.sh --backup
sudo bash install.sh --restore /path/to/vast-revenue-monitor-backup-*.tar.gz
sudo systemctl status vast-balance.service --no-pager -l
sudo journalctl -u vast-balance.service -n 100 -l --no-pager
sudo systemctl stop vast-balance.service
cd /opt/vast-revenue-monitor && sudo -u vast-revenue-monitor \
  .venv/bin/python balance.py --config config.json --once
sudo systemctl start vast-balance.service
```

Confirm that the hourly report and a testable record notification reach Discord.
Then verify a clean lifecycle (the `N` answer creates a backup before removal):

```bash
sudo bash uninstall.sh
test ! -e /opt/vast-revenue-monitor
test ! -e /etc/systemd/system/vast-balance.service
! id vast-revenue-monitor
sudo bash install.sh
sudo systemctl is-active vast-balance.service
```
...
Vast Revenue Monitor is an unofficial monitoring tool for Vast.ai. Revenue figures
are estimates derived from changes observed in the Vast.ai account balance and may
not match final settlement, billing, payout, or tax records. This project is not
affiliated with, sponsored by, or endorsed by Vast.ai.
