"""Safe interactive updater for supported mutable settings."""
...
def main(path: Path) -> int:
    ...
    try:
        ...
        if not isinstance(current_detailed_report, bool):
            raise ValueError("current detailed report value must be true or false")
        ...
        current_detailed_label = "enabled" if current_detailed_report else "disabled"
        while True:
            choice = input(f"Choice [{current_detailed_label}]: ").strip().lower()
            if not choice:
                break
        ...
        if (
            ...
            and detailed_report == current_detailed_report
            and "detailed_report" in data
        ):
