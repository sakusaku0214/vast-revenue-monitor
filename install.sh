...
if [[ "${MODE}" == "reconfigure" ]]; then
  exec bash "${APP_DIR}/reconfigure.sh"
fi
...
if [[ -f "${APP_DIR}/config.json" ]]; then
  ...
  DETAILED_REPORT="${DETAILED_REPORT:-false}"
  NOTIFICATION_LANGUAGE="${NOTIFICATION_LANGUAGE:-en}"
  if [[ -t 0 ]]; then
    read -r -p 'Weekly revenue goal (USD) [1000]: ' weekly_goal_answer
    WEEKLY_GOAL_USD="${weekly_goal_answer:-${WEEKLY_GOAL_USD}}"
    ...
    read -r -p 'Choice [1]: ' language_choice
    case "${language_choice:-1}" in 1) NOTIFICATION_LANGUAGE=en ;; 2) NOTIFICATION_LANGUAGE=ja ;; *) fail "Invalid language choice" ;; esac
  fi
